...
void ccm_prepare_first_ctr_blk(AES_BYTE counter[], const AES_BYTE nonce[], int nonce_len, int payload_len_store_size);
void ccm_prepare_first_format_blk(AES_BYTE buf[], int assoc_len, int payload_len, int payload_len_store_size, int mac_len, const AES_BYTE nonce[], int nonce_len);
void ccm_format_assoc_data(AES_BYTE buf[], int *end_of_buf, const AES_BYTE assoc[], int assoc_len);
void ccm_format_payload_data(AES_BYTE buf[], int *end_of_buf, const AES_BYTE payload[], int payload_len);
...
static const AES_BYTE aes_sbox[16][16] = {
...
static const AES_BYTE aes_invsbox[16][16] = {
...
static const AES_BYTE gf_mul[256][6] = {
...
void xor_buf(const AES_BYTE in[], AES_BYTE out[], size_t len)
{
	size_t idx;
...
* AES - CBC
*******************/
int aes_encrypt_cbc(const AES_BYTE in[], size_t in_len, AES_BYTE out[], const AES_WORD key[], int keysize, const AES_BYTE iv[])
{
	AES_BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_encrypt_cbc_mac(const AES_BYTE in[], size_t in_len, AES_BYTE out[], const AES_WORD key[], int keysize, const AES_BYTE iv[])
{
	AES_BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
* AES - CTR
*******************/
void increment_iv(AES_BYTE iv[], int counter_size)
{
	int idx;
...
void aes_encrypt_ctr(const AES_BYTE in[], size_t in_len, AES_BYTE out[], const AES_WORD key[], int keysize, const AES_BYTE iv[])
{
	size_t idx = 0, last_block_length;
	AES_BYTE iv_buf[AES_BLOCK_SIZE], out_buf[AES_BLOCK_SIZE];
...
void aes_decrypt_ctr(const AES_BYTE in[], size_t in_len, AES_BYTE out[], const AES_WORD key[], int keysize, const AES_BYTE iv[])
{
...
int aes_encrypt_ccm(const AES_BYTE payload[], AES_WORD payload_len, const AES_BYTE assoc[], unsigned short assoc_len,
                    const AES_BYTE nonce[], unsigned short nonce_len, AES_BYTE out[], AES_WORD *out_len,
                    AES_WORD mac_len, const AES_BYTE key_str[], int keysize)
{
	AES_BYTE temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], mac[16], *buf;
	...
	buf = (AES_BYTE*)malloc(payload_len + assoc_len + 48 /*Round both payload and associated data up a block size and add an extra block.*/);
	if (! buf)
		return(FALSE);
...
int aes_decrypt_ccm(const AES_BYTE ciphertext[], AES_WORD ciphertext_len, const AES_BYTE assoc[], unsigned short assoc_len,
                    const AES_BYTE nonce[], unsigned short nonce_len, AES_BYTE plaintext[], AES_WORD *plaintext_len,
                    AES_WORD mac_len, int *mac_auth, const AES_BYTE key_str[], int keysize)
{
	AES_BYTE temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], mac[16], mac_buf[16], *buf;
	...
	buf = (AES_BYTE*)malloc(assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48);
	if (! buf)
		return(FALSE);
...
void aes_key_setup(const AES_BYTE key[], AES_WORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	AES_WORD temp,Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
...
void AddRoundKey(AES_BYTE state[][4], const AES_WORD w[])
{
	AES_BYTE subkey[4];
...
void SubBytes(AES_BYTE state[][4])
{
...
void InvSubBytes(AES_BYTE state[][4])
{
...
void ShiftRows(AES_BYTE state[][4])
{
	int t;
...
void InvShiftRows(AES_BYTE state[][4])
{
	int t;
...
void MixColumns(AES_BYTE state[][4])
{
	AES_BYTE col[4];
...
void InvMixColumns(AES_BYTE state[][4])
{
	AES_BYTE col[4];
...
void aes_encrypt(const AES_BYTE in[], AES_BYTE out[], const AES_WORD key[], int keysize)
{
	AES_BYTE state[4][4];
...
void aes_decrypt(const AES_BYTE in[], AES_BYTE out[], const AES_WORD key[], int keysize)
{
	AES_BYTE state[4][4];
...
void print_state(AES_BYTE state[][4])
{
	int idx,idx2;
