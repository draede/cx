...
#include "CX/Archive/IGenerator.hpp"
...
public:
	...
	virtual Status Generate(OUTPUT out, const Object &object)
	{
		...
		Print(out, "#include \"CX/Archive/BLOB.hpp\"\n");
		Print(out, "#include \"CX/Archive/MemberType.hpp\"\n");
		Print(out, "#include \"CX/Archive/DataWriter.hpp\"\n");
		Print(out, "#include \"CX/Archive/DataReader.hpp\"\n");
		Print(out, "#include \"CX/Archive/IArchivable.hpp\"\n");
...
#pragma warning(pop)
		...
		Print(out, "class {1} : public CX::Archive::IArchivable\n", sType);
		Print(out, "{{\n");
		Print(out, "public:\n");
		Print(out, "\n");
		...
		Print(out, "\tvirtual CX::Status Read(CX::Archive::IReader *pReader, const CX::Char *szName = NULL)\n");
		Print(out, "\t{{\n");
		...
		for (Object::MembersVector::const_iterator iter = object.GetMembers().begin(); iter != object.GetMembers().end(); ++iter)
		{
			...
			switch (iter->GetMemberType())
			{
				case MemberType_Scalar:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataReader<CX::Archive::MemberType_Scalar, {1}>::Read("
				...
				case MemberType_Vector:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataReader<CX::Archive::MemberType_Vector, {1}>::Read("
					      "pReader, m_{2}, \"{2}\")).IsNOK())\n", sKeyType, iter->GetName());
					Print(out, "\t\t{{\n");
					Print(out, "\t\t\treturn status;\n");
					Print(out, "\t\t}\n");
				...
				case MemberType_Set:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataReader<CX::Archive::MemberType_Set, {1}>::Read("
					      "pReader, m_{2}, \"{2}\")).IsNOK())\n", sKeyType, iter->GetName());
					Print(out, "\t\t{{\n");
					Print(out, "\t\t\treturn status;\n");
					Print(out, "\t\t}\n");
				...
				case MemberType_Map:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataReader<CX::Archive::MemberType_Map, {1}, {2}>::Read("
		...
		Print(out, "\tvirtual CX::Status Write(CX::Archive::IWriter *pWriter, const CX::Char *szName = NULL) const\n");
		Print(out, "\t{{\n");
		Print(out, "\t\tCX::Status status;\n");
		Print(out, "\n");
		...
		for (Object::MembersVector::const_iterator iter = object.GetMembers().begin(); iter != object.GetMembers().end(); ++iter)
		{
			...
			switch (iter->GetMemberType())
			{
				case MemberType_Scalar:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataWriter<CX::Archive::MemberType_Scalar, {1}>::Write("
				...
				case MemberType_Vector:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataWriter<CX::Archive::MemberType_Vector, {1}>::Write("
					      "pWriter, m_{2}, \"{2}\")).IsNOK())\n", sKeyType, iter->GetName());
					Print(out, "\t\t{{\n");
					Print(out, "\t\t\treturn status;\n");
					Print(out, "\t\t}\n");
				...
				case MemberType_Set:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataWriter<CX::Archive::MemberType_Set, {1}>::Write("
					      "pWriter, m_{2}, \"{2}\")).IsNOK())\n", sKeyType, iter->GetName());
					Print(out, "\t\t{{\n");
					Print(out, "\t\t\treturn status;\n");
					Print(out, "\t\t}\n");
				...
				case MemberType_Map:
				{
					Print(out, "\t\tif ((status = CX::Archive::DataWriter<CX::Archive::MemberType_Map, {1}, {2}>::Write("
